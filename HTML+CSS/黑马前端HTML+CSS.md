...
1. 像素（px）：像素是最常用的CSS单位之一，表示屏幕上的一个像素点。通常情况下，1个像素的大小是相对固定的，因此像素单位通常用于确定固定大小的元素。
2. EM（em）：EM是相对于元素的字体大小来测量的单位。例如，如果一个元素的字体大小为16像素，那么1em就等于16像素。如果元素的字体大小被设置为20像素，那么1em就等于20像素。EM单位通常用于设计响应式布局，因为它们可以自适应文本的大小。
3. EX（ex）：EX是相对于元素字体中小写字母x的高度来测量的单位。通常情况下，1个ex的大小等于字体大小的一半。EX单位通常用于垂直对齐元素的文本。
4. 百分比（%）：百分比单位是相对于包含块的大小来测量的。例如，如果一个元素的宽度设置为50%，那么它的宽度将等于它的包含块的50%。百分比单位通常用于实现响应式设计。
5. REM（rem）：REM是相对于根元素（即文档顶部的HTML元素）字体大小来测量的单位。如果根元素的字体大小为16像素，那么1rem等于16像素。REM单位通常用于响应式设计，因为它们可以方便地基于文档的根元素来设置大小。
